...
const tsByDefinition = function (name, definition) {
  ...
  if (definition.type === 'object') {
    ...
    Object.keys(properties).forEach(function (prop) {
     //  swagger  文件不规范，都没有写那些是必须返回的，理论上应该从swagger 读取的
...
const generateTypeof = function (userParams, name, definitions, dependentVos, affectedMapping) {
  ...
  // rimraf.sync(path.resolve(userParams.tsType, ownName + '.ts'));
...
module.exports = function (apis, definitions, userParams, fileName) {
  ...
  const codeBody = apis
    .map((api, apiIndex) => {
      ...
      const returns = `  return request(${[url, configParamState]
        .filter(filterEmpty)
        .join(', ')});`;
  ...
  const statements = `
${typeDefCode ? `/**\n${typeDefCode}\n */\n` : ''}import * as request from '${userParams.httpBase}';
${codeBody}
`;

  const tsCode = `${tsImports.length ? `${tsImports.join('')}\n` : ''}import${userParams.importStar ? ' * as' : ''} request from '${userParams.httpBase}';\n${codeBody}`;
